...
class JFormFieldACFOSM extends JFormFieldText
{
	...
	public function getInput()
	{
		...
		$html .= '<div class="osm-field-settings">';
			...
			$tooltip_value = is_array($decoded_value) && isset($decoded_value['tooltip']) ? $decoded_value['tooltip'] : '';
			if ($this->element['show_tooltip'] == '1')
			{
			$tooltip_name = $this->formControl . '[' . $this->group . '][' . $this->fieldname . '][tooltip]';
			$html .= '<div class="control-group">';
				$html .= '<label class="control-label" for="' . $this->id . '_tooltip_label">' . JText::_('ACF_OSM_TOOLTIP_LABEL') . '</label>';
				$html .= '<div class="controls"><textarea id="' . $this->id . '_tooltip_label" rows="3" name="' . $tooltip_name . '" placeholder="' . JText::_('ACF_OSM_TOOLTIP_LABEL_HINT') . '">' . $tooltip_value . '</textarea></div>';
			$html .= '</div>';
			}
